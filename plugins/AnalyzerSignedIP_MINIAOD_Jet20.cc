...
class AnalyzerSignedIP_MINIAOD_Jet20 : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   public:
      explicit AnalyzerSignedIP_MINIAOD_Jet20(const edm::ParameterSet&);
      ~AnalyzerSignedIP_MINIAOD_Jet20();
...
//      std::vector<int> genP_id_MM;
      ...
      std::vector<int> jet_Nseeds;
      std::vector<int> jet_NJetseeds;
      ...
      std::vector<double> seed_3D_signedIp;
      std::vector<double> seed_3D_signedSip;
      std::vector<double> seed_2D_signedIp;
      std::vector<double> seed_2D_signedSip;
...
//       std::vector<double> nearTracks_3D_signedIp;
//       std::vector<double> nearTracks_3D_signedSip;
//       std::vector<double> nearTracks_2D_signedIp;
//       std::vector<double> nearTracks_2D_signedSip;
...
AnalyzerSignedIP_MINIAOD_Jet20::AnalyzerSignedIP_MINIAOD_Jet20(const edm::ParameterSet& iConfig)

{
   ...
   usesResource("TFileService");
    ...
    tree->Branch("jet_flavour",&jet_flavour);
    tree->Branch("jet_Nseeds",&jet_Nseeds);    
    tree->Branch("jet_NJetseeds",&jet_NJetseeds);
    ...
    tree->Branch("seed_3D_signedIp", &seed_3D_signedIp);
    tree->Branch("seed_3D_signedSip", &seed_3D_signedSip);
    tree->Branch("seed_2D_signedIp", &seed_2D_signedIp);
    tree->Branch("seed_2D_signedSip", &seed_2D_signedSip);
...
//     tree->Branch("nearTracks_3D_signedIp", &nearTracks_3D_signedIp);
//     tree->Branch("nearTracks_3D_signedSip", &nearTracks_3D_signedSip);
//     tree->Branch("nearTracks_2D_signedIp", &nearTracks_2D_signedIp);
//     tree->Branch("nearTracks_2D_signedSip", &nearTracks_2D_signedSip);
...
AnalyzerSignedIP_MINIAOD_Jet20::~AnalyzerSignedIP_MINIAOD_Jet20()
{
...
void
AnalyzerSignedIP_MINIAOD_Jet20::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    seed_2D_ip.clear();
    seed_2D_sip.clear();
    seed_3D_signedIp.clear();
    seed_3D_signedSip.clear();
    seed_2D_signedIp.clear();
    seed_2D_signedSip.clear();
    seed_JetMatch.clear();
...
//     nearTracks_3D_signedIp.clear(); //signed only in jet
//     nearTracks_3D_signedSip.clear();
//     nearTracks_2D_signedIp.clear();
//     nearTracks_2D_signedSip.clear();
...
//         std::cout << " " << index << " == " << it - selectedTracks.begin() <<  " " << genP_index[index]   << "  " << genP_index.size() <<std::endl;
        ...
        count_seeds=0;
        if (jet_Nseeds.size() >0) min_size+=jet_Nseeds.back();
...
//            std::cout << "new seed " <<  it-selectedTracks.begin() << " ref " << it->trackBaseRef().key()  << " " << ip.second.value() << " " << ip.second.significance() << " " << it->track().hitPattern().trackerLayersWithMeasurement() << " " << it->track().pt() << " " << it->track().eta() << std::endl;
            ...
            for(i_sip = trial_vector.begin(); (i_sip != trial_vector.end()); ++i_sip) {
                ...
                std::cout<<"jets   "<<jet_Nseeds.size() <<"  n  "<< min_size<<std::endl;
                
                if ((i_sip - trial_vector.begin())>=0){                
                    if (*i_sip<ipSigned.second.significance() && int(sip_sorting_index)>=min_size) {at_end=0; break;}               
            ...
            seed_pt.insert(seed_pt.begin()+sip_sorting_index+at_end, it->track().pt());
            seed_eta.insert(seed_eta.begin()+sip_sorting_index+at_end, it->track().eta());
            seed_phi.insert(seed_phi.begin()+sip_sorting_index+at_end, it->track().phi());
            seed_mass.insert(seed_mass.begin()+sip_sorting_index+at_end, masses[it-selectedTracks.begin()]);
            
            seed_dz.insert(seed_dz.begin()+sip_sorting_index+at_end, it->track().dz());
            seed_dxy.insert(seed_dxy.begin()+sip_sorting_index+at_end, it->track().dxy());
            seed_3D_ip.insert(seed_3D_ip.begin()+sip_sorting_index+at_end, ip.second.value());
            seed_3D_sip.insert(seed_3D_sip.begin()+sip_sorting_index+at_end, ip.second.significance());
            seed_2D_ip.insert(seed_2D_ip.begin()+sip_sorting_index+at_end, ip2d.second.value());
            seed_2D_sip.insert(seed_2D_sip.begin()+sip_sorting_index+at_end, ip2d.second.significance());
            seed_3D_signedIp.insert(seed_3D_signedIp.begin()+sip_sorting_index+at_end, ipSigned.second.value());
            seed_3D_signedSip.insert(seed_3D_signedSip.begin()+sip_sorting_index+at_end, ipSigned.second.significance());
            seed_2D_signedIp.insert(seed_2D_signedIp.begin()+sip_sorting_index+at_end, ip2dSigned.second.value());
            seed_2D_signedSip.insert(seed_2D_signedSip.begin()+sip_sorting_index+at_end, ip2dSigned.second.significance());
            
                       
            seed_JetMatch.insert(seed_JetMatch.begin()+sip_sorting_index+at_end, seed_jet_match_i);
...
//             pair = ClosestJet(*it, jet_eta,jet_phi);
            ...
            seed_chi2reduced.insert(seed_chi2reduced.begin()+sip_sorting_index+at_end, it->track().normalizedChi2());
            seed_nPixelHits.insert(seed_nPixelHits.begin()+sip_sorting_index+at_end, it->track().hitPattern().numberOfValidPixelHits());
            seed_nHits.insert(seed_nHits.begin()+sip_sorting_index+at_end, it->track().hitPattern().numberOfValidHits());  
            
            if(seed_jet_match_i>=0){
                
            //GlobalVector direction(jet_px[seed_jet_match_i], jet_py[seed_jet_match_i], jet_pz[seed_jet_match_i]); 
            std::pair<double, Measurement1D> jet_distance =IPTools::jetTrackDistance(*it, direction, pv);
            seed_jetAxisDistance.insert(seed_jetAxisDistance.begin()+sip_sorting_index+at_end, std::fabs(jet_distance.second.value())); 
            ...
            if (closest.isValid()) seed_jetAxisDlength.insert(seed_jetAxisDlength.begin()+sip_sorting_index+at_end, (closest.globalPosition() - pvp).mag()); 
            else seed_jetAxisDlength.insert(seed_jetAxisDlength.begin()+sip_sorting_index+at_end, -99); 
            
            }
            
           else{seed_jetAxisDistance.insert(seed_jetAxisDistance.begin()+sip_sorting_index+at_end, -100.); seed_jetAxisDlength.insert(seed_jetAxisDlength.begin()+sip_sorting_index+at_end, -100.);  }
...
//            }
            ...
            nearTracks_pt.insert(nearTracks_pt.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].pt);
            nearTracks_eta.insert(nearTracks_eta.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].eta);
            nearTracks_phi.insert(nearTracks_phi.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].phi);
            nearTracks_dz.insert(nearTracks_dz.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].dz);
            nearTracks_dxy.insert(nearTracks_dxy.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].dxy);
            nearTracks_mass.insert(nearTracks_mass.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].mass);
            nearTracks_3D_ip.insert(nearTracks_3D_ip.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].t3Dip);
            nearTracks_3D_sip.insert(nearTracks_3D_sip.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].t3Dsip);
            nearTracks_2D_ip.insert(nearTracks_2D_ip.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].t2Dip);
            nearTracks_2D_sip.insert(nearTracks_2D_sip.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].t2Dsip);
            nearTracks_PCAdist.insert(nearTracks_PCAdist.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].dist);
            nearTracks_PCAdsig.insert(nearTracks_PCAdsig.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].dsig);
            nearTracks_PCAonSeed_x.insert(nearTracks_PCAonSeed_x.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].PCA_sx);
            nearTracks_PCAonSeed_y.insert(nearTracks_PCAonSeed_y.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].PCA_sy);
            nearTracks_PCAonSeed_z.insert(nearTracks_PCAonSeed_z.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].PCA_sz);
            nearTracks_PCAonSeed_xerr.insert(nearTracks_PCAonSeed_xerr.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].PCA_sxerr);
            nearTracks_PCAonSeed_yerr.insert(nearTracks_PCAonSeed_yerr.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].PCA_syerr);
            nearTracks_PCAonSeed_zerr.insert(nearTracks_PCAonSeed_zerr.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].PCA_szerr);
            nearTracks_PCAonTrack_x.insert(nearTracks_PCAonTrack_x.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].PCA_tx);
            nearTracks_PCAonTrack_y.insert(nearTracks_PCAonTrack_y.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].PCA_ty);
            nearTracks_PCAonTrack_z.insert(nearTracks_PCAonTrack_z.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].PCA_tz);
            nearTracks_PCAonTrack_xerr.insert(nearTracks_PCAonTrack_xerr.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].PCA_txerr);
            nearTracks_PCAonTrack_yerr.insert(nearTracks_PCAonTrack_yerr.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].PCA_tyerr);
            nearTracks_PCAonTrack_zerr.insert(nearTracks_PCAonTrack_zerr.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].PCA_tzerr);
            nearTracks_dotprodTrack.insert(nearTracks_dotprodTrack.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].dotprodTrack);
            nearTracks_dotprodSeed.insert(nearTracks_dotprodSeed.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].dotprodSeed);
            nearTracks_dotprodTrackSeed2D.insert(nearTracks_dotprodTrackSeed2D.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].dotprodTrackSeed2D);
            nearTracks_dotprodTrackSeed3D.insert(nearTracks_dotprodTrackSeed3D.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].dotprodTrackSeed3D);
            nearTracks_dotprodTrackSeedVectors2D.insert(nearTracks_dotprodTrackSeedVectors2D.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].dotprodTrackSeedVectors2D);
            nearTracks_dotprodTrackSeedVectors3D.insert(nearTracks_dotprodTrackSeedVectors3D.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].dotprodTrackSeedVectors3D);
            nearTracks_PCAonSeed_pvd.insert(nearTracks_PCAonSeed_pvd.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].seedPCA_pv);
            nearTracks_PCAonTrack_pvd.insert(nearTracks_PCAonTrack_pvd.begin()+sip_sorting_index*20+at_end*20+i,nearTracks[i].trackPCA_pv);
        ...
        jet_Nseeds.push_back(count_seeds);
        jet_NJetseeds.push_back(count_seeds+min_size);
        }
        }
   tree->Fill();
...
int AnalyzerSignedIP_MINIAOD_Jet20::funzionePippo (int vivaLaVita) {
return 1;
}


std::vector<float> AnalyzerSignedIP_MINIAOD_Jet20::firstHadronInChain(pat::PackedGenParticle gParticle, const reco::Candidate* mother){
...
std::vector<int> AnalyzerSignedIP_MINIAOD_Jet20::seedToAllJets20Matching(reco::TransientTrack seed, std::vector<double> jet_eta, std::vector<double> jet_phi, std::vector<double> jet_pt)
{
...
int AnalyzerSignedIP_MINIAOD_Jet20::seedToJetMatching(reco::TransientTrack seed, std::vector<double> jet_eta, std::vector<double> jet_phi) {
  float angular_distance =  0.4;
  float dist_temp;
  int index = -1;
  for (unsigned int i = 0; i <jet_eta.size(); i++) {
...
int AnalyzerSignedIP_MINIAOD_Jet20::seedToJet30Matching(reco::TransientTrack seed, std::vector<double> jet_eta, std::vector<double> jet_phi, std::vector<double> jet_pt) {
  float angular_distance =  0.4;
  float dist_temp;
  int index = -1;
  for (unsigned int i = 0; i <jet_eta.size(); i++) {
...
std::pair<int,float> AnalyzerSignedIP_MINIAOD_Jet20::ClosestJet(reco::TransientTrack seed, std::vector<double> jet_eta, std::vector<double> jet_phi) {
...
double AnalyzerSignedIP_MINIAOD_Jet20::trackEff(std::vector<pat::PackedGenParticle> genP, std::vector<reco::TransientTrack> tracks, double &BChain_eff, double &DChain_eff, double &B_eff,
      double &low_pt_eff, double &high_pt_eff, std::vector<double> &minGenChiSquare){
...
bool AnalyzerSignedIP_MINIAOD_Jet20::genLevelMM(std::vector<pat::PackedGenParticle> genP, std::vector<reco::TransientTrack> tracks, std::vector<int>& genPnumbers, std::vector<double>& assigned_chisquares){ 
...
int AnalyzerSignedIP_MINIAOD_Jet20::genMap(std::vector<pat::PackedGenParticle> genP, std::vector<reco::TransientTrack> tracks, std::vector<int>& genPnumbers, std::vector<double>& allChi){
...
int AnalyzerSignedIP_MINIAOD_Jet20::genLevel(std::vector<pat::PackedGenParticle> genP, reco::TransientTrack seed, std::vector<int>& MomFlav, std::vector<int>& BChain, std::vector<double>& allChi){
...
void 
AnalyzerSignedIP_MINIAOD_Jet20::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void 
AnalyzerSignedIP_MINIAOD_Jet20::endJob() 
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
AnalyzerSignedIP_MINIAOD_Jet20::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(AnalyzerSignedIP_MINIAOD_Jet20);
