...
class AnalyzerSignedIP_MINIAOD_JetbyJet : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   public:
      explicit AnalyzerSignedIP_MINIAOD_JetbyJet(const edm::ParameterSet&);
      ~AnalyzerSignedIP_MINIAOD_JetbyJet();
...
AnalyzerSignedIP_MINIAOD_JetbyJet::AnalyzerSignedIP_MINIAOD_JetbyJet(const edm::ParameterSet& iConfig)
...
AnalyzerSignedIP_MINIAOD_JetbyJet::~AnalyzerSignedIP_MINIAOD_JetbyJet()
{
...
void
AnalyzerSignedIP_MINIAOD_JetbyJet::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
int AnalyzerSignedIP_MINIAOD_JetbyJet::funzionePippo (int vivaLaVita) {
return 1;
}


std::vector<float> AnalyzerSignedIP_MINIAOD_JetbyJet::firstHadronInChain(pat::PackedGenParticle gParticle, const reco::Candidate* mother){
...
std::vector<int> AnalyzerSignedIP_MINIAOD_JetbyJet::seedToAllJets20Matching(reco::TransientTrack seed, std::vector<double> jet_eta, std::vector<double> jet_phi, std::vector<double> jet_pt)
{
...
int AnalyzerSignedIP_MINIAOD_JetbyJet::seedToJetMatching(reco::TransientTrack seed, std::vector<double> jet_eta, std::vector<double> jet_phi) {
  float angular_distance =  0.4;
  float dist_temp;
  int index = -1;
  for (unsigned int i = 0; i <jet_eta.size(); i++) {
...
int AnalyzerSignedIP_MINIAOD_JetbyJet::seedToJet30Matching(reco::TransientTrack seed, std::vector<double> jet_eta, std::vector<double> jet_phi, std::vector<double> jet_pt) {
  float angular_distance =  0.4;
  float dist_temp;
  int index = -1;
  for (unsigned int i = 0; i <jet_eta.size(); i++) {
...
std::pair<int,float> AnalyzerSignedIP_MINIAOD_JetbyJet::ClosestJet(reco::TransientTrack seed, std::vector<double> jet_eta, std::vector<double> jet_phi) {
...
double AnalyzerSignedIP_MINIAOD_JetbyJet::trackEff(std::vector<pat::PackedGenParticle> genP, std::vector<reco::TransientTrack> tracks, double &BChain_eff, double &DChain_eff, double &B_eff,
      double &low_pt_eff, double &high_pt_eff, std::vector<double> &minGenChiSquare){
...
bool AnalyzerSignedIP_MINIAOD_JetbyJet::genLevelMM(std::vector<pat::PackedGenParticle> genP, std::vector<reco::TransientTrack> tracks, std::vector<int>& genPnumbers, std::vector<double>& assigned_chisquares){ 
...
int AnalyzerSignedIP_MINIAOD_JetbyJet::genMap(std::vector<pat::PackedGenParticle> genP, std::vector<reco::TransientTrack> tracks, std::vector<int>& genPnumbers, std::vector<double>& allChi){
...
int AnalyzerSignedIP_MINIAOD_JetbyJet::genLevel(std::vector<pat::PackedGenParticle> genP, reco::TransientTrack seed, std::vector<int>& MomFlav, std::vector<int>& BChain, std::vector<double>& allChi){
...
void 
AnalyzerSignedIP_MINIAOD_JetbyJet::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void 
AnalyzerSignedIP_MINIAOD_JetbyJet::endJob() 
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
AnalyzerSignedIP_MINIAOD_JetbyJet::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(AnalyzerSignedIP_MINIAOD_JetbyJet);
