...
class AnalyzerSignedIP_trackSelectionCuts : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   public:
      explicit AnalyzerSignedIP_trackSelectionCuts(const edm::ParameterSet&);
      ~AnalyzerSignedIP_trackSelectionCuts();
   ...
   private:
      ...
      std::vector<int> seedToAllJets30Matching(reco::TransientTrack seed, std::vector<double> jet_eta, std::vector<double> jet_phi, std::vector<double> jet_pt);
      ...
      int seedSisters(std::vector<reco::GenParticle> genP, int genP_index,  std::vector<trackVars2> nearTracks);
...
AnalyzerSignedIP_trackSelectionCuts::AnalyzerSignedIP_trackSelectionCuts(const edm::ParameterSet& iConfig)

{
   ...
   usesResource("TFileService");
    ...
    tree->Branch("jet_seedIndex",&jet_seedIndex);
...
AnalyzerSignedIP_trackSelectionCuts::~AnalyzerSignedIP_trackSelectionCuts()
{
...
void
AnalyzerSignedIP_trackSelectionCuts::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
//        std::cout << " " << it->track().pt() << " " << it->track().eta() <<  " " << it->track().phi()   << std::endl;
        ...
        for(std::vector<TransientTrack>::const_iterator it = selectedTracks.begin(); it != selectedTracks.end(); it++){
        std::vector<int> jet_index_array = seedToAllJets30Matching(*it, jet_eta,jet_phi, jet_pt); 
        
        for (unsigned j_i=0; j_i < jet_index_array.size(); j_i++) {
        
            std::cout << "array of indexes " << jet_index_array[j_i] << std::endl;
        int seed_jet_match_i=jet_index_array[j_i];
...
//         int seed_jet_match_i=seedToJetMatching(*it, jet_eta,jet_phi); 
        ...
        std::pair<bool,Measurement1D> ip2d = IPTools::absoluteTransverseImpactParameter(*it, pv);
            ...
            it->track().normalizedChi2()<5. &&  seed_jet_match_i>=0 && std::fabs(it->track().dxy(pv.position())) < 2 &&
...
//            std::cout << "new seed " <<  it-selectedTracks.begin() << " ref " << it->trackBaseRef().key()  << " " << ip.second.value() << " " << ip.second.significance() << " " << it->track().hitPattern().trackerLayersWithMeasurement() << " " << it->track().pt() << " " << it->track().eta() << std::endl;
            ...
            seed_3D_signedIp.push_back(ipSigned.second.value());
            seed_3D_signedSip.push_back(ipSigned.second.significance());
            seed_2D_signedIp.push_back(ip2dSigned.second.value());
            seed_2D_signedSip.push_back(ip2dSigned.second.significance());
...
//            }
            ...
            nearTracks_MC_fromSeedChain.push_back(same_chain/*riempire la variabile*/);
            }
...
int AnalyzerSignedIP_trackSelectionCuts::funzionePippo (int vivaLaVita) {
return 1;
}


std::vector<float> AnalyzerSignedIP_trackSelectionCuts::firstHadronInChain(reco::GenParticle gParticle, edm::RefVector<std::vector<reco::GenParticle> > mothers){
...
std::vector<int> AnalyzerSignedIP_trackSelectionCuts::seedToAllJets30Matching(reco::TransientTrack seed, std::vector<double> jet_eta, std::vector<double> jet_phi, std::vector<double> jet_pt)
{
...
int AnalyzerSignedIP_trackSelectionCuts::seedToJetMatching(reco::TransientTrack seed, std::vector<double> jet_eta, std::vector<double> jet_phi) {
  float angular_distance =  0.4;
  float dist_temp;
  int index = -1;
  for (unsigned int i = 0; i <jet_eta.size(); i++) {
...
int AnalyzerSignedIP_trackSelectionCuts::seedToJet30Matching(reco::TransientTrack seed, std::vector<double> jet_eta, std::vector<double> jet_phi, std::vector<double> jet_pt) {
  float angular_distance =  0.4;
  float dist_temp;
  int index = -1;
  for (unsigned int i = 0; i <jet_eta.size(); i++) {
...
std::pair<int,float> AnalyzerSignedIP_trackSelectionCuts::ClosestJet(reco::TransientTrack seed, std::vector<double> jet_eta, std::vector<double> jet_phi) {
...
int  AnalyzerSignedIP_trackSelectionCuts::seedSisters(std::vector<reco::GenParticle> genP, int genP_index, std::vector<trackVars2> nearTracks)
{
...
double AnalyzerSignedIP_trackSelectionCuts::trackEff(std::vector<reco::GenParticle> genP, std::vector<reco::TransientTrack> tracks, double &BChain_eff, double &DChain_eff, double &B_eff,
      double &low_pt_eff, double &high_pt_eff, std::vector<double> &minGenChiSquare){
...
bool AnalyzerSignedIP_trackSelectionCuts::genLevelMM(std::vector<reco::GenParticle> genP, std::vector<reco::TransientTrack> tracks, std::vector<int>& genPnumbers, std::vector<double>& assigned_chisquares){ 
...
int AnalyzerSignedIP_trackSelectionCuts::genMap(std::vector<reco::GenParticle> genP, std::vector<reco::TransientTrack> tracks, std::vector<int>& genPnumbers, std::vector<double>& allChi){
...
int AnalyzerSignedIP_trackSelectionCuts::genLevel(std::vector<reco::GenParticle> genP, reco::TransientTrack seed, std::vector<int>& MomFlav, std::vector<int>& BChain, std::vector<double>& allChi){
...
void 
AnalyzerSignedIP_trackSelectionCuts::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void 
AnalyzerSignedIP_trackSelectionCuts::endJob() 
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
AnalyzerSignedIP_trackSelectionCuts::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(AnalyzerSignedIP_trackSelectionCuts);
